...
import type { RoomSettings } from "@/lib/game/settings";
import type { FeedKind, RoomStatus, RoundStatus, Stroke, TurnResult, Avatar } from "@/lib/game/types";
...
export interface RoomRow {
  ...
  used_words: string[];
  /** Absolute turn number (1-based) that pays double, or null. */
  double_points_turn: number | null;
  /** Deadline for the current phase (pick / draw / intermission). */
...
export interface PlayerRow {
  ...
  /** Set by the Freeze power-up; guesses are rejected until it passes. */
