...
import { normalizeWord, shuffle } from "./text";
...
/** Concrete nouns a seven-year-old can draw. Default pack for mixed rooms. */
...
/** Idioms, abstractions and pop culture. Host opt-in; tuned for adults. */
...
/** Custom host packs have no tiers, so length and word count stand in. */
...
/**
 * Three choices, one per difficulty tier, skipping words already used this
 * game. Falls back across tiers so a small custom pack still works.
 */
...
/** Categories rotate each game so a long session stays varied. */
