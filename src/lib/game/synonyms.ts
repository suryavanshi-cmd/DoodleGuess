...
/**
 * Offline near-synonym table for the bundled word packs.
 *
 * This is deliberately a curated map rather than a WordNet import: the full
 * dump is megabytes, most of it irrelevant to a 200-word game, and its looser
 * senses ("cat" -> "computerized axial tomography") make for bad rulings. Ships
 * with the app, loads as a plain object, costs nothing per guess.
 *
 * Matching is bidirectional — listing it once is enough.
 */
...
function buildIndex(): Map<string, Set<string>> {
  ...
  for (const [word, list] of Object.entries(NEAR_SYNONYMS)) {
    for (const synonym of list) {
      link(word, synonym);
      link(synonym, word);
      // Two synonyms of the same word are near enough to each other too.
...
/** Is `guess` a listed near-synonym of `word`? Pure lookup, no I/O. */
